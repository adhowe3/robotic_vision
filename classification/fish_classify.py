...
def save_dataset_sample(dataloader, filename="fish_sample.png"):
    # 1. Grab a single batch of images
    images, labels = next(iter(dataloader))
    
    # 2. Un-normalize the images for viewing
    # (Since we used Mean=[0.485, 0.456, 0.406] and Std=[0.229, 0.224, 0.225])
    sample_imgs = images.clone()
    for i, (mean, std) in enumerate(zip([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])):
        sample_imgs[:, i, :, :] = sample_imgs[:, i, :, :] * std + mean

    # 3. Create a grid and save it
    grid = make_grid(sample_imgs, nrow=8) # 8 images per row
...
if __name__ == "__main__":
    ...
    train_transform = transforms.Compose([
        transforms.Resize((64,64)), # MAYBE TRY MORE RECTANGLE SHAPE?
    ...
    train_dataset = datasets.ImageFolder(root="fish_species/train", transform=train_transform)
    test_dataset = datasets.ImageFolder(root='fish_species/test', transform=test_transform)
    ...
    #show the images
    save_dataset_sample(train_loader, "check_fish.png")
